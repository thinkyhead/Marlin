...
void TMC5130_init(uint8_t csPin, uint8_t irun, uint8_t ihold, uint8_t stepper_direction) {
  ...
  switch(csPin){
    case XAXIS_CS:
      SBI(DRV_EN_X_DDR, DRV_EN_X);
      TMC5130_disableDriver(X_AXIS);
      break;
    case YAXIS_CS:
      SBI(DRV_EN_Y_DDR, DRV_EN_Y);
      TMC5130_disableDriver(Y_AXIS);
      break;
    case ZAXIS_CS:
      SBI(DRV_EN_Z_DDR, DRV_EN_Z);
      TMC5130_disableDriver(Z_AXIS);
      break;
    case E0AXIS_CS:
      SBI(DRV_EN_E0_DDR, DRV_EN_E0);
 ...
 * @return  none
 *****************************************************************************/
void TMC5130_enableDriver(const AxisEnum axis) {
  switch(axis) { // Active low!
    case X_AXIS: CBI(DRV_EN_X_PORT, DRV_EN_X); break;
    case Y_AXIS: CBI(DRV_EN_Y_PORT, DRV_EN_Y); break;
    case Z_AXIS: CBI(DRV_EN_Z_PORT, DRV_EN_Z); break;
    case E_AXIS: CBI(DRV_EN_E0_PORT, DRV_EN_E0); break;
    default: break;
  }
}
 ...
 * @return  none
 *****************************************************************************/
void TMC5130_disableDriver(const AxisEnum axis) {
  switch(axis) { // Active low!
    case X_AXIS: SBI(DRV_EN_X_PORT, DRV_EN_X); break;
    case Y_AXIS: SBI(DRV_EN_Y_PORT, DRV_EN_Y); break;
    case Z_AXIS: SBI(DRV_EN_Z_PORT, DRV_EN_Z); break;
    case E_AXIS: SBI(DRV_EN_E0_PORT, DRV_EN_E0); break;
    default: break;
  }
}
 ...
 * @return  none
 *****************************************************************************/
void TMC5130_homing(const AxisEnum axis) {
  unsigned int sw_register;
  uint8_t motor_direction;
  uint32_t stallguardthreshold;
  int homing_retract, homing_speed, axis_to_home;
