...
#if !defined(TARGET_LPC1768) && DISABLED(IS_ANKERMAKE_M5) && ANY( \
    ENDSTOPPULLDOWNS, \
 ...
 */
  ...
  #elif !ANKER_MAKE_API && !HAS_JUNCTION_DEVIATION && defined(DEFAULT_EJERK)
...
#undef _TMC_MICROSTEP_IS_VALID
  ...
  #define Z_ENDSTOP_INVERTING !AXIS_DRIVER_TYPE(Z,TMC2209)
    ...
    #if   X_SENSORLESS && X_HOME_TO_MIN && NONE(ENDSTOPPULLUP_XMIN, ENDSTOPPULLDOWN_XMIN)
      #error "SENSORLESS_HOMING requires ENDSTOPPULLUP_XMIN or ENDSTOPPULLDOWN_XMIN (or ENDSTOPPULLUPS or ENDSTOPPULLDOWNS) when homing to X_MIN."
    #elif X_SENSORLESS && X_HOME_TO_MAX && NONE(ENDSTOPPULLUP_XMAX, ENDSTOPPULLDOWN_XMAX)
      #error "SENSORLESS_HOMING requires ENDSTOPPULLUP_XMAX or ENDSTOPPULLDOWN_XMAX (or ENDSTOPPULLUPS or ENDSTOPPULLDOWNS) when homing to X_MAX."
    #elif Y_SENSORLESS && Y_HOME_TO_MIN && NONE(ENDSTOPPULLUP_YMIN, ENDSTOPPULLDOWN_YMIN)
      #error "SENSORLESS_HOMING requires ENDSTOPPULLUP_YMIN or ENDSTOPPULLDOWN_YMIN (or ENDSTOPPULLUPS or ENDSTOPPULLDOWNS) when homing to Y_MIN."
    #elif Y_SENSORLESS && Y_HOME_TO_MAX && NONE(ENDSTOPPULLUP_YMAX, ENDSTOPPULLDOWN_YMAX)
      #error "SENSORLESS_HOMING requires ENDSTOPPULLUP_YMAX or ENDSTOPPULLDOWN_YMAX (or ENDSTOPPULLUPS or ENDSTOPPULLDOWNS) when homing to Y_MAX."
    #elif Z_SENSORLESS && Z_HOME_TO_MIN && NONE(ENDSTOPPULLUP_ZMIN, ENDSTOPPULLDOWN_ZMIN)
      #error "SENSORLESS_HOMING requires ENDSTOPPULLUP_ZMIN or ENDSTOPPULLDOWN_ZMIN (or ENDSTOPPULLUPS or ENDSTOPPULLDOWNS) when homing to Z_MIN."
    #elif Z_SENSORLESS && Z_HOME_TO_MAX && NONE(ENDSTOPPULLUP_ZMAX, ENDSTOPPULLDOWN_ZMAX)
      #error "SENSORLESS_HOMING requires ENDSTOPPULLUP_ZMAX or ENDSTOPPULLDOWN_ZMAX (or ENDSTOPPULLUPS or ENDSTOPPULLDOWNS) when homing to Z_MAX."
    #elif LINEAR_AXES >= 4 && I_SENSORLESS && I_HOME_TO_MAX && NONE(ENDSTOPPULLUP_IMAX, ENDSTOPPULLDOWN_IMAX)
      #error "SENSORLESS_HOMING requires ENDSTOPPULLUP_IMAX or ENDSTOPPULLDOWN_IMAX (or ENDSTOPPULLUPS or ENDSTOPPULLDOWNS) when homing to I_MAX."
    #elif LINEAR_AXES >= 5 && J_SENSORLESS && J_HOME_TO_MAX && NONE(ENDSTOPPULLUP_JMAX, ENDSTOPPULLDOWN_JMAX)
      #error "SENSORLESS_HOMING requires ENDSTOPPULLUP_JMAX or ENDSTOPPULLDOWN_JMAX (or ENDSTOPPULLUPS or ENDSTOPPULLDOWNS) when homing to J_MAX."
    #elif LINEAR_AXES >= 6 && K_SENSORLESS && K_HOME_TO_MAX && NONE(ENDSTOPPULLUP_KMAX, ENDSTOPPULLDOWN_KMAX)
      #error "SENSORLESS_HOMING requires ENDSTOPPULLUP_KMAX or ENDSTOPPULLDOWN_KMAX (or ENDSTOPPULLUPS or ENDSTOPPULLDOWNS) when homing to K_MAX."
    #endif
  #endif
  ...
  #elif ENDSTOP_NOISE_THRESHOLD
    #if ENABLED(ENDSTOP_NOISE_Z) && defined(Z_STALL_SENSITIVITY)
     #error "SENSORLESS_HOMING 'Z' is incompatible with ENDSTOP_NOISE_Z."
    #elif ENABLED(ENDSTOP_NOISE_X) && defined(X_STALL_SENSITIVITY)
     #error "SENSORLESS_HOMING 'X' is incompatible with ENDSTOP_NOISE_X."
    #elif ENABLED(ENDSTOP_NOISE_Y) && defined(Y_STALL_SENSITIVITY)
     #error "SENSORLESS_HOMING 'Y' is incompatible with Y_STALL_SENSITIVITY."
    #endif
