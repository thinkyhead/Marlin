...
static const dwin_charmap_t g_dwin_charmap_device[] PROGMEM = {
  // sorted by uchar:
...
static const dwin_charmap_t g_dwin_charmap_common[] PROGMEM = {
  {IV('¡'), 'i', 0}, // A1
  {IV('¢'), 'c', 0}, // A2
...
/* return v1 - v2 */
static int dwin_charmap_compare(dwin_charmap_t *v1, dwin_charmap_t *v2) {
  return (v1->uchar < v2->uchar) ? -1 : (v1->uchar > v2->uchar) ? 1 : 0;
}

static int pf_bsearch_cb_comp_dwinmap_pgm(void *userdata, size_t idx, void * data_pin) {
...
void lcd_moveto(const lcd_uint_t col, const lcd_uint_t row) {
  // TODO: Maintain a text cursor position
...
int lcd_put_wchar_max(wchar_t c, pixel_len_t max_length) {
  ...
  //if (max_length < dwin_font.width) return 0;

  uint8_t charsize = dwin_font.width;
  ...
  if (c < 255) {
    ...
    if (ret >= 0) {
      ...
      str[0] = localval.idx;
      if (max_length >= dwin_font.width * 2 && localval.idx2 > 0) {
        charsize += dwin_font.width;
        str[1] = localval.idx2;
      }
    }
  }
  // Draw the char(s) at the cursor and advance the cursor
  DWIN_Draw_String(false, dwin_font.solid, dwin_font.index, dwin_font.fg, dwin_font.bg, cursor.x, cursor.y, str);
  cursor.x += charsize;
  return charsize;
}
