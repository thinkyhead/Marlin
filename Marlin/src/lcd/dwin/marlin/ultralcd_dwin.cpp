...
void MarlinUI::draw_status_screen() {
  ...
  static bool mark_as_selected(const uint8_t row, const bool sel) {
    ...
    if (sel) {
        ...
        DWIN_Draw_HLine(Color_Selected, 0, row_y1 + 1, DWIN_WIDTH);
        DWIN_Draw_HLine(Color_Selected, 0, row_y2 + 2, DWIN_WIDTH);
      #else
        DWIN_Draw_Box(1, Color_Selected, 0, row_y1 + 2, DWIN_WIDTH, MENU_FONT_HEIGHT - 1);
      #endif
    }
    return true;
  }
  ...
  void MenuItem_static::draw(const uint8_t row, PGM_P const pstr, const uint8_t style/*=SS_DEFAULT*/, const char * const vstr/*=nullptr*/) {
    ...
    //set_dwin_text_bg((style & SS_INVERT) ? Color_Selected : Color_Bg_Black);
  ...
  void MenuItemBase::_draw(const bool sel, const uint8_t row, PGM_P const pstr, const char, const char post_char) {
    ...
    //set_dwin_text_bg(sel ? Color_Selected : Color_Bg_Black);
  ...
  inline void draw_boxed_string(const dwin_coord_t x, const dwin_coord_t y, PGM_P const pstr, const bool inv) {
    ...
    if (inv) {
      DWIN_Draw_Box(1, Color_Selected, bx - 1, by - (MENU_FONT_ASCENT) + 1, bw + 2, MENU_FONT_HEIGHT - 1);
