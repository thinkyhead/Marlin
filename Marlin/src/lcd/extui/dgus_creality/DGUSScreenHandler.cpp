...
void DGUSScreenHandler::LoadSettings(const char* buff) {
  ...
  TERN_(HAS_COLOR_LEDS_PREFERENCES, leds.set_color(Settings.LastLEDColor));
...
void DGUSScreenHandler::StoreSettings(char* buff) {
  ...
  TERN_(HAS_COLOR_LEDS_PREFERENCES, Settings.LastLEDColor = leds.color);
...
void DGUSScreenHandler::HandleUserConfirmationPopUp(uint16_t VP, const char* line1, const char* line2, const char* line3, const char* line4, bool l1, bool l2, bool l3, bool l4) {
  if (current_screen == DGUSLCD_SCREEN_CONFIRM)
    PopToOldScreen(); // Already showing a pop up, so we need to cancel that first.
...
void DGUSScreenHandler::DGUSLCD_SendPrintTimeToDisplay(DGUS_VP_Variable &var) {
  ...
  const bool shouldDisplay = ui.remaining_time == 0;
  ...
  if (elapsed == last_elapsed) return;
...
void DGUSScreenHandler::DGUSLCD_SendPrintTimeRemainingToDisplay(DGUS_VP_Variable &var) {
    ...
    const uint32_t remaining_time = ui.remaining_time;
    if (lastRemainingTime == remaining_time) return;

    const bool has_remaining_time = remaining_time != 0;

    // Update display of SPs (toggle between large and small print timer)
    dgusdisplay.WriteVariable(VP_HideRemainingTime_Ico, has_remaining_time ? ICON_REMAINING_VISIBLE : ICON_REMAINING_HIDDEN);
...
bool DGUSScreenHandler::HandlePendingUserConfirmation() {
  if (!ExtUI::isWaitingOnUser()) return false;
...
void DGUSScreenHandler::SetViewMeshLevelState() {
  ...
  void DGUSScreenHandler::OnMeshLevelingUpdate(const int8_t x, const int8_t y, const float z) {
    SERIAL_ECHOLNPGM("X: ", x, "; Y: ", y, "; Index ", MeshLevelIndex, "; Icon ", MeshLevelIconIndex);

    UpdateMeshValue(x, y, z);

    if (MeshLevelIndex < 0) return; // We're not leveling
...
uint16_t CreateRgb(double h, double s, double v) {
  ...
  switch (i) {
    ...
    default:
    case 5: out.r = in.v; out.g = p; out.b = q; break;
  }
...
void DGUSScreenHandler::HandleScreenVersionMismatchLEDFlash() {
  ...
  if (ELAPSED(ms, next_event_ms)) {
    ...
    TERN_(HAS_COLOR_LEDS, leds.set_color(caselight.on ? LEDColorRed() : LEDColorOff()));
  }
}
...
void DGUSScreenHandler::HandleLiveAdjustZ(DGUS_VP_Variable &var, void *val_ptr, const_float_t scalingFactor) {
  ...
  TERN_(HAS_BED_PROBE, RequestSaveSettings()); // Without a probe the Z offset is applied using baby offsets, which aren't saved anyway.
  ScreenHandler.ForceCompleteUpdate();
  ScreenHandler.skipVP = var.VP; // don't overwrite value the next update time as the display might autoincrement in parallel
}
...
#if HAS_PROBE_SETTINGS && HAS_BED_PROBE
...
void DGUSScreenHandler::PopToOldScreen() {
  ...
  if (past_screens[0] != 0) {
    ...
    GotoScreen(ExtUI::isPrinting() ? DGUSLCD_SCREEN_PRINT_RUNNING : DGUSLCD_SCREEN_MAIN, false);
}
...
void DGUSScreenHandler::GotoScreen(DGUSLCD_Screens screen, bool save_current_screen) {
  if (current_screen == screen) return; // Ignore this request
