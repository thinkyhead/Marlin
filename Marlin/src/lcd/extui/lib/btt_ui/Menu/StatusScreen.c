...
void drawTemperature(void)
{
  ...
  //TOOL / EXT
    ...
    //BED
    lvIcon.lines[0].text = (u8 *)heatDisplayID[TSC_BED];
    sprintf(tempstr, "%d℃", heatGetCurrentTemp(TSC_BED));
    sprintf(tempstr2, "%d℃", heatGetTargetTemp(TSC_BED));
    lvIcon.lines[1].text = (u8 *)tempstr;
    lvIcon.lines[2].text = (u8 *)tempstr2;
  ...
  sprintf(tempstr, "   X: %.2f   Y: %.2f   Z: %.2f   ", coordinateGetAxisActual(TSC_X_AXIS), coordinateGetAxisActual(TSC_Y_AXIS), coordinateGetAxisActual(TSC_Z_AXIS));
...
void drawStatusScreenMsg(void) {
  ...
  ICON_CustomReadDisplay(rect_of_keySS[17].x0, rect_of_keySS[17].y0, INFOBOX_WIDTH, INFOBOX_HEIGHT, INFOBOX_ADDR);
...
void menuStatus(void) {
  ...
  drawTemperature();
  drawStatusScreenMsg();
  menuSetFrontCallBack(menuStatusCallBack);
}
