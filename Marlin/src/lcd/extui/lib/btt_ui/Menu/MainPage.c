...
void unifiedMenu(void)
{
  ...
  const MENUITEMS unifiedPageItems = {
    // title
    LABEL_MAINMENU,
    // icon                         label
  ...
  menuSetFrontCallBack(unifiedMenuCallBack);
}

void classicMenuCallBack(void)
{
  KEY_VALUES key_num = menuKeyGetValue();
  switch(key_num)
  {
...
void classicMenu(void)
{
  ...
  menuSetFrontCallBack(classicMenuCallBack);
}
