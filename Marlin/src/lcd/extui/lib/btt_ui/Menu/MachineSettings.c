...
const MENUITEMS eepromSettingsItems = {
  // title
...
void menuCustom(void)
{
  ...
  W25Qxx_ReadBuffer((uint8_t*)&tempcodes,CUSTOM_GCODE_ADDR,sizeof(CUSTOM_GCODES));
  ...
  menuSetFrontCallBack(menuCustomCallBack);
}

void menuRGBSettingsCallBack(void)
{
  KEY_VALUES key_num = menuKeyGetValue();
  switch(key_num)
  {
    case KEY_ICON_0:  //Red
      storeCmd("M150 S0 R255 U0 B0 P255\n");
      break;

    case KEY_ICON_1:  //Green
      storeCmd("M150 S0 R0 U255 B0 P255\n");
      break;

    case KEY_ICON_2:  //Blue
      storeCmd("M150 S0 R0 U0 B255 P255\n");
      break;

    case KEY_ICON_3:  //White
      storeCmd("M150 S0 R255 U255 B255 P255\n");
      break;

    case KEY_ICON_4:  //Turn Off
      storeCmd("M150 S0 R0 U0 B0 P0\n");
      break;
...
void menuEepromSettingsCallBack(void)
{
  KEY_VALUES  key_num = menuKeyGetValue();
  switch(key_num)
  {
...
void menuEepromSettings(void)
{
  ...
  menuSetFrontCallBack(menuEepromSettingsCallBack);
}
#endif

void menuMachineSettingsCallBack(void)
{
  KEY_VALUES key_num = menuKeyGetValue();
  switch(key_num)
  {
...
{
  // 1 title, ITEM_PER_PAGE items (icon + label)
  const MENUITEMS machineSettingsItems = {
    // title
  ...
  menuSetFrontCallBack(menuMachineSettingsCallBack);
}
