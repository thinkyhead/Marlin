...
extern volatile int8_t encoderDiff; // Updated in update_buttons, added to encoderPosition every LCD update
...
{
  uint8_t newbutton = 0;
  if(encoder_ReadStep(LCD_ENCA_INDEX)) newbutton |= EN_A;
  if(encoder_ReadStep(LCD_ENCB_INDEX)) newbutton |= EN_B;
...
uint8_t LCD_ReadTouch(void)
{
  ...
  if(!XPT2046_Read_Pen() && CTime < OS_GetTimeMs())
  {
    ...
    if (ex > LCD_FREE_WIDTH)
    {
      if ((sy > ey) && ey != 0)
      {
        if (sy - ey > BTT_LCD_HEIGHT / 9 && sy - ey < BTT_LCD_HEIGHT / 7) //7-5
        {
          sy = ey;
          return 2;
        }
      ...
      {
        if (ey - sy > BTT_LCD_HEIGHT / 9 && ey - sy < BTT_LCD_HEIGHT / 7)
        {
          sy = ey;
          return 3;
        }
