...
//#define SUTOADD      //Press and hold the key to add and subtract, which is turned off by default
...
extern float tsc_x_inc;
extern float tsc_x_offset;
extern float tsc_y_inc;
extern float tsc_y_offset;
...
uint16_t  KEY_Keep_pressing(uint8_t total_rect,const GUI_RECT* menuRect);
