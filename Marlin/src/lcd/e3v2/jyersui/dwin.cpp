      ...
      case HostActions:
        ...
        switch(item) {
          ...
          case HOSTACTIONS_1:
            ...
            else if (!strcmp(action1, "-") == 0)
              hostui.action(F(action1));
            break;
          case HOSTACTIONS_2:
            if (draw)
              Draw_Menu_Item(row, ICON_File, action2);
            else if (!strcmp(action2, "-") == 0)
              hostui.action(F(action2));
            break;
          case HOSTACTIONS_3:
            if (draw)
              Draw_Menu_Item(row, ICON_File, action3);
            else if (!strcmp(action3, "-") == 0)
              hostui.action(F(action3));
            break;
        }
        break;
...
void MarlinUI::init() {
  ...
  for (uint16_t t = 0; t <= 100; t += 2) {
    DWIN_ICON_Show(ICON, ICON_Bar, 15, bartop);
    DWIN_Draw_Rectangle(1, Color_Bg_Black, 15 + t * 242 / 100, bartop, 257, bartop + 20);
    DWIN_UpdateLCD();
    delay(20);
  }
