...
const float *Get_NOZZLE_TO_PROBE_OFFSET(void) {
  ...
  return nozzle_board_type == NOZZLE_TYPE_NEW ? &nozzle_offset_new[0] : &nozzle_offset_old[0];
}
...
void sys_err_parse(uint32_t err) {
  if (TEST(err, SYS_ERR_04_OCI_MOS_INPUT_H)) {
  ...
  if (TEST(err, SYS_ERR_05_HEATER_FAN_ACTIVE))
    MYSERIAL2.printLine("Error:Heater Fan feedback\n");

  if (TEST(err, SYS_ERR_06_HEATER_FAN_INACTIVE))
    MYSERIAL2.printLine("Error:Heater Fan feedback\n");

  if (TEST(err, SYS_ERR_07_MODEL_FAN_ACTIVE))
    MYSERIAL2.printLine("Error:Model Fan feedback\n");

  if (TEST(err, SYS_ERR_08_MODEL_FAN_INACTIVE))
    MYSERIAL2.printLine("Error:Model Fan feedback\n");

  if (TEST(err, SYS_ERR_10_ADC_RANGE_EXCEED))
    MYSERIAL2.printLine("heater adc range exceed\n");

  if (TEST(err, SYS_ERR_11_TEMP_MAX_LIMIT)) {
  ...
  if (TEST(err, SYS_ERR_12_TEMP_MIN_LIMIT)) {
  ...
  if (TEST(err, SYS_ERR_13_20S_HEAT_TIMEOUT)) {
...
static void uart_nozzle_rx_callback(gcp_msg_t *gcp_msg) {
  ...
  if (gcp_msg->src_module != GCP_MODULE_02_NOZZLE) return;
  if (gcp_msg->dst_module != GCP_MODULE_01_MARLIN) return;

  dev_probe_flg = 1;
  nozzle_rst_cnt = 0;
  uart_nozzle_rx.start_ms = millis();
  ...
  switch (gcp_msg->cmd) {
    ...
    case GCP_CMD_28_ENV_T_ALARM:
      ...
      if (gcp_msg->content[0] == 1) {
        if (uart_nozzle_rx.mos_err_flg == 0) {
          ...
          uart_nozzle_rx.mos_err_start_ms = millis();
        }
      }
    ...
    case GCP_CMD_34_LEVELING_VAL_READ:
      anker_probe_set.leveling_value = PACK_LE_16(&gcp_msg->content[0]);
      MYSERIAL2.printLine("echo:M3020 V%d\r\n", anker_probe_set.leveling_value);
      break;
...
static void uart_nozzle_rx_disconnect_check(void) {
  int gap = millis() - uart_nozzle_rx.start_ms;
...
static void uart_nozzle_info_echo(void) {
  if (uart_nozzle_info.setup != 1) return;
...
void hotend_mos_err_polling(void) {
  if (uart_nozzle_rx.mos_err_flg == 0) return;
  if (int(millis() - uart_nozzle_rx.mos_err_start_ms) <= 5000) return;
