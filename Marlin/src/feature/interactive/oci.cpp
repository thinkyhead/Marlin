...
//static void oci_latch_re_enable_callback() {
//  oci_latch_clear();
//  oci.mutex = 0;
//}

//static void oci_in_isr() {
//  if (oci.mutex != 0) return;
//
//  oci.mutex = 1;
...
static void oci_delay() {
  volatile uint32_t count = 500;
  while (count--) { /* nada */ }
}

//static void oci_mos_check_callback() {
//  uint8_t level;
//
//  if (oci.mutex == 1) return;
//  if (READ_OUT(OCI_HOTBED_PWR_PIN) == 0) return;
//  if (Temperature::temp_bed.target != 0) return;
//
  ...
  if (OCI_FEED_READ() == 1) {
    ...
    if (OCI_FEED_READ() == 1) {
      ...
      if (hotbed_mos_check.flg == 0) {
        ...
        hotbed_mos_check.start_ms = millis();
      }
    }
  }
...
void hotbed_mos_err_polling() {
  ...
  if (int(millis() - hotbed_mos_check.start_ms) > 5000) {
    fatal_err = 1;
...
uint8_t hw_ver_read() { return hw_ver; }
