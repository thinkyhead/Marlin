...
#if ANY(ANKER_ALIGN, ANKER_ALIGN_ONLY_Z)
...
void Anker_Align::auto_align() {
  ...
  const float __align_allowed = TERN(ADAPT_DETACHED_NOZZLE, IS_new_nozzle_board() ? NOZZLE_TYPE_NEW_ALIGN_ALLOWED : ALIGN_ALLOWED, ALIGN_ALLOWED);
  anker_align.init();
  ...
  for (num = 0; num < ALIGN_NUM; num++) {
    ...
    if (ABS(z1 - z2) <= __align_allowed) {
      ...
      SERIAL_ECHOLNPGM("echo:anlign ok!");
