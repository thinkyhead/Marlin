...
#if BOTH(ANKER_MAKE_API, ANKER_PAUSE_FUNC)
...
static void anker_pause_continue() {
  anker_pause_info_t *p_info = get_anker_pause_info();
  p_info->pause_cmd_state = ANKER_PAUSE_CMD_CONTINUE;
}

static void anker_pause_end() {
  anker_pause_info_t *p_info = get_anker_pause_info();
...
static void anker_pause_save_buf_init() {
  unsigned int i = 0;
  ...
  for (i = 0; i < BUFSIZE; i++) memset(p_info->save_queue_buf.commands[i].buffer, 0, MAX_CMD_SIZE);
}
...
static void anker_pause_block_deal() {
  anker_pause_info_t *p_info = get_anker_pause_info();
...
anker_pause_info_t *get_anker_pause_info() {
  static anker_pause_info_t anker_pause_info = {0};
  return &anker_pause_info;
}

#endif // ANKER_MAKE_API && ANKER_PAUSE_FUNC
