    ...
    get_destination_from_command(); // For X Y Z E F
      ...
      if (MIN_AUTORETRACT <= MAX_AUTORETRACT) {
        ...
        if (fwretract.autoretract_enabled && parser.seen('E') && !(parser.seen('X') || parser.seen('Y') || parser.seen('Z'))) {
          const float echange = tool.destination[E_AXIS] - tool.position[E_AXIS];
          // Is this a retract or recover move?
          if (WITHIN(ABS(echange), MIN_AUTORETRACT, MAX_AUTORETRACT) && fwretract.retracted[tool.index] == (echange > 0.0)) {
            tool.position[E_AXIS] = tool.destination[E_AXIS]; // Hide a G1-based retract/recover from calculations
