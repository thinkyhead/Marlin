...
void GcodeSuite::M900() {
  ...
  if (newK != oldK) {
  ...
  #if ENABLED(ANKER_E_SMOOTH) && EXTRUDERS < 2
    if (parser.seenval('A')) { //add by Anan.huang
      ...
      const float smooth_accel = parser.value_float();  // mm/s2 1733mm/s2
      const uint32_t extra_K_acc_steps_per_s2 = smooth_accel * planner.settings.axis_steps_per_mm[E_AXIS];// get Smooth acceleration of K-value velocity(steps/s2)
