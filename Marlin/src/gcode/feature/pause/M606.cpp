...
void GcodeSuite::M606() {
  ...
  if (resume_e) do_pause_e_move(resume_e, MMM_TO_MMS(DTM_UNLOAD_FEEDRATE));
  ...
  // Load filament
  do_pause_e_move(DTM_LOAD_LENGTH, MMM_TO_MMS(DTM_LOAD_FEEDRATE));
  ...
  // Purge
  do_pause_e_move(DTM_PURGE_LENGTH, MMM_TO_MMS(DTM_PURGE_FEEDRATE));
  ...
  if (resume_position[E_AXIS] < 0) do_pause_e_move(resume_position[E_AXIS], MMM_TO_MMS(DTM_UNLOAD_FEEDRATE));
