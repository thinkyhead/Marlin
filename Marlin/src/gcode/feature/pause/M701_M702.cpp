...
void GcodeSuite::M701() {
  ...
  // Show initial "wait for load" message
    ...
    uint8_t current_tool_before_filament_change = tool.index;
    if (tool.index != target_extruder)
      tool_change(target_extruder, 0, false);
  #endif

  // Lift Z axis
  if (park_point.z > 0)
    do_blocking_move_to_z(MIN(tool.position[Z_AXIS] + park_point.z, Z_MAX_POS), NOZZLE_PARK_Z_FEEDRATE);
  ...
  // Restore Z axis
  if (park_point.z > 0)
    do_blocking_move_to_z(MAX(tool.position[Z_AXIS] - park_point.z, 0), NOZZLE_PARK_Z_FEEDRATE);
...
void GcodeSuite::M702() {
  ...
  // Unload filament
    ...
    mmu2.unload();
      ...
      if (!parser.seenval('T')) {
        HOTEND_LOOP() {
          if (e != tool.index) tool_change(e, 0, false);
