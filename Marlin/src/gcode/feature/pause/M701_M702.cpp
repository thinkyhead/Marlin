...
void GcodeSuite::M701() {
  ...
  // Load filament
    ...
    constexpr float slow_load_length = FILAMENT_CHANGE_SLOW_LOAD_LENGTH;
        const float fast_load_length = ABS(parser.seenval('L') ? parser.value_axis_units(E_AXIS) : fc_settings[active_extruder].load_length),
                    purge_length = ABS(parser.seenval('P') ? parser.value_float() : ADVANCED_PAUSE_PURGE_LENGTH);
