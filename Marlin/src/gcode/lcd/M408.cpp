...
void GcodeSuite::M408() {
  ...
  SERIAL_ECHO(IS_SD_PRINTING() ? "\"P\"" : "\"I\"");
  ...
  json_key(PSTR("homed"));
  SERIAL_ECHOPAIR("[", int(TEST(axis_homed, X_AXIS)));
  SERIAL_ECHOPAIR(",", int(TEST(axis_homed, Y_AXIS)));
  SERIAL_ECHOPAIR(",", int(TEST(axis_homed, Z_AXIS)));
  SERIAL_CHAR(']');
    ...
    if (ui.status_message[0]) {
      ...
      SERIAL_ECHOPAIR("\"", ui.status_message);
      SERIAL_CHAR('"');
    }
  #endif
