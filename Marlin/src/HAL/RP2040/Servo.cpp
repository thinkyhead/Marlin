...
int8_t libServo::attach(const int pin) {
  ...
  auto result = rp2040_servo.attach(servo_pin);
...
int8_t libServo::attach(const int pin, const int min, const int max) {
  if (servoCount >= MAX_SERVOS) return -1;
  if (pin > 0) servo_pin = pin;
  auto result = rp2040_servo.attach(servo_pin, min, max);
  fixServoTimerInterruptPriority();
  return result;
}
...
void libServo::pause() {
  was_attached_before_pause = rp2040_servo.attached();
  if (was_attached_before_pause) {
    value_before_pause = rp2040_servo.read();
    rp2040_servo.detach();
  }
}
