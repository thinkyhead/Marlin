...
void HAL_timer_start(const uint8_t timer_num, const uint32_t frequency) {
  switch (timer_num) {
    ...
    //
    // Step Timer – GPT1 - Compare Interrupt OCR2 - Reset Mode
    //
    case MF_TIMER_STEP:
      ...
      GPT1_OCR2 = (GPT1_TIMER_RATE / frequency) - 1;
    ...
    //
    // Temperature Timer – GPT2 - Compare Interrupt OCR2 - Reset Mode
    //
    case MF_TIMER_TEMP:
      // 24MHz mode off – Use peripheral clock (150MHz)
      CCM_CSCMR1 &= ~CCM_CSCMR1_PERCLK_CLK_SEL;
      ...
      // Compare value – the number of clocks between edges
      GPT2_OCR2 = (GPT2_TIMER_RATE / frequency) - 1;
...
void HAL_timer_isr_prologue(const uint8_t timer_num) {
  switch (timer_num) {
    case MF_TIMER_STEP: GPT1_SR = GPT_IR_OF2IE; break;   // clear OF2
    case MF_TIMER_TEMP: GPT2_SR = GPT_IR_OF2IE; break;
  }
  asm volatile("dsb");
}
