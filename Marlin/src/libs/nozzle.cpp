...
#include "point_t.h"
  ...
  void Nozzle::stroke(const point_t &start, const point_t &end, const uint8_t &strokes) {
      ...
      const float ix = tool.position[X_AXIS], iy = tool.position[Y_AXIS], iz = tool.position[Z_AXIS];
    #endif
  ...
  void Nozzle::park(const uint8_t z_action, const point_t &park /*= NOZZLE_PARK_POINT*/) {
    ...
    switch (z_action) {
      ...
      case 2: // Raise by Z-park height
        do_blocking_move_to_z(MIN(tool.position[Z_AXIS] + park.z, Z_MAX_POS), fr_z);
        break;

      default: // Raise to at least the Z-park height
        do_blocking_move_to_z(MAX(park.z, tool.position[Z_AXIS]), fr_z);
    }
