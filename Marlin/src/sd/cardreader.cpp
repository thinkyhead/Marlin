...
void CardReader::printListing(SdFile parent, const char * const prepend/*=nullptr*/) {
  ...
  while (parent.readDir(&p, longFilename) > 0) {
    if (DIR_IS_SUBDIR(&p)) {
      ...
      // Get a child folder object and dive recursively into it.
...
void CardReader::removeFile(const char * const name) {
  ...
  SdFile *itsDir;
  const char * const fname = diveToFile(false, itsDir, name);
  if (!fname) return;

  #if ENABLED(SDCARD_READONLY)
    SERIAL_ECHOLNPAIR("Deletion failed (read-only), File: ", fname, ".");
  #else
    if (file.remove(itsDir, fname)) {
...
const char* CardReader::diveToFile(const bool update_cwd, SdFile*& inDirPtr, const char * const path, const bool echo/*=false*/) {
  ...
  if (path[0] == '/') {               // Starting at the root directory?
    inDirPtr = &root;
    atom_ptr++;
    DEBUG_ECHOLNPAIR("diveToFile: CWD to root: ", hex_address((void*)inDirPtr));
  ...
  DEBUG_ECHOLNPAIR("diveToFile: startDirPtr = ", hex_address((void*)startDirPtr));
  ...
  while (atom_ptr) {
    ...
    if (inDirPtr != startDirPtr) {
      DEBUG_ECHOLNPAIR("diveToFile: closing inDirPtr: ", hex_address((void*)inDirPtr));
      inDirPtr->close();
    }

    // inDirPtr now subDir
    inDirPtr = sub;
    DEBUG_ECHOLNPAIR("diveToFile: inDirPtr = sub: ", hex_address((void*)inDirPtr));
  ...
  if (update_cwd) {
    workDir = *inDirPtr;
    DEBUG_ECHOLNPAIR("diveToFile: final workDir = ", hex_address((void*)inDirPtr));
