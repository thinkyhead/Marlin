...
inline void gcode_G28() {
  ...
  inline void gcode_G30() {
     ...
     *          n=4 probes 4 center points, towers, opposite-towers and itermediate points 
    ...
    inline void gcode_G33() {
      ...
      float test_precision, zero_std_dev = 999.0;

      int probe_points = DELTA_CALIBRATION_DEFAULT_POINTS,
          verbose_level = 1;

      if (code_seen('C')) {
        probe_points = code_value_int();
        if ((probe_points < 1 || probe_points > 4) && probe_points != -2)
          probe_points = DELTA_CALIBRATION_DEFAULT_POINTS;
        zero_std_dev = 999.00;
      }
      if (code_seen('V')) {
      ...
      if (abs(probe_points) > 1) {
        ...
        if (endstop_adj[0] >= 0) SERIAL_CHAR('+');
        SERIAL_PROTOCOL_F(endstop_adj[0], 2);
        SERIAL_PROTOCOLPGM("  Ey:");
        if (endstop_adj[1] >= 0) SERIAL_CHAR('+');
        SERIAL_PROTOCOL_F(endstop_adj[1], 2);
        SERIAL_PROTOCOLPGM("  Ez:");
        if (endstop_adj[2] >= 0) SERIAL_CHAR('+');
        SERIAL_PROTOCOL_F(endstop_adj[2], 2);
      ...
      do { // start iterations
        ...
        int step_axis = 4;
        if (probe_points >= 3) {
          for (int axis = 9; axis > 0; axis -= step_axis) { // uint8_t starts endless loop
            z_at_pt[0] += probe_pt(
        ...
        int start = (probe_points == -2) ? 3 : 1;

        if (probe_points != 1) {
          for (int axis = start; axis < 13; axis += step_axis)
        ...
        if (zero_std_dev < test_precision) {
          ...
          #define Z0700(I) ZP(2*H_FACTOR/3.00, I)
        ...
        if (test_precision != 0.0) { // !forced end
          if (zero_std_dev >= test_precision) { // end iterations
            ...
            SERIAL_PROTOCOLLNPGM("                                   roling back 1");
            lcd_setstatus("Calibration OK");
          SERIAL_EOL;
          }
          else { // !end iterations
