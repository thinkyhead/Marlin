...
static void setup_for_endstop_move() {
  ...
  static float probe_pt(float x, float y, float z_before, ProbeAction probe_action=ProbeDeployAndStow, int verbose_level=1) {
      ...
      SERIAL_EOL;
      print_xyz("> do_blocking_move_to Z", current_position[X_AXIS], current_position[Y_AXIS], z_before);
    #endif
  ...
  inline void gcode_G29() {
    ...
    feedrate = homing_feedrate[Z_AXIS];
      ...
      if (!dryrun) {
        ...
        current_position[Z_AXIS] = -zprobe_zoffset + Z_RAISE_AFTER_PROBING + (z_tmp - real_z); // The difference is added to current position and sent to planner.
        sync_plan_position();
