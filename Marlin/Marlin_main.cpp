...
inline void buffer_line_to_destination(const float &fr_mm_s) {
  ...
  void prepare_uninterpolated_move_to_destination(const float fr_mm_s=0) {
      ...
      if ( current_position[X_AXIS] == destination[X_AXIS]
        ...
        && current_position[E_CART] == destination[E_CART] // E or Spindle
      ) return;
        ...
        if (tool_type == TOOL_TYPE_PICKER) {
          ...
          destination[S_AXIS] -= (delta[A_AXIS] + delta[B_AXIS] - planner.get_axis_position_mm(A_AXIS) - planner.get_axis_position_mm(B_AXIS));
        }
      #endif
...
void clean_up_after_endstop_or_probe_move() {
   ...
   */
  vector_3 tool_point_to_end_point(const float &rx, const float &ry) {

    // Simply can't reach the given point
    if (HYPOT2(rx, ry) > sq(L1 + L2 + tool_offset_length))
      return vector_3();
    ...
    const float curr_tool_offset_length = tool_offset_length,
...
inline void gcode_G4() {
  ...
  inline void gcode_G8() {
    ...
    float a = 90, b = 0;
    forward_kinematics_SCARA(a, b);
    current_position[X_AXIS] = cartes[X_AXIS];
    current_position[Y_AXIS] = cartes[Y_AXIS];
    current_position[Z_AXIS] = Z_HOME_POS;
    axis_homed = axis_known_position = xyz_bits;
    SYNC_PLAN_POSITION_KINEMATIC();
    SERIAL_ECHO_START();
    SERIAL_ECHOLNPGM("MakerArm Home X=" STRINGIFY(X_HOME_POS) " Y=" STRINGIFY(Y_HOME_POS) " Z=" STRINGIFY(Z_HOME_POS));
    report_current_position();
  }
  ...
  void refresh_tool_offset_factors() {
    ...
    if (tool_offset[X_AXIS]) {
      const float ylen = L2 + tool_offset[Y_AXIS];
      tool_offset_angle = DEGREES(ATAN2(ylen, tool_offset[X_AXIS])) - 90;
      tool_offset_length = HYPOT(ylen, tool_offset[X_AXIS]) - L2;
    }
    else {
      tool_offset_angle = 0;
  ...
  inline void gcode_G10() {
    const float old_z_offset = tool_offset[Z_AXIS];
...
void home_all_axes() { gcode_G28(true); }
  ...
  inline void gcode_G30() {
      ...
      const float xpos = parser.linearval('X', probe.x),
                  ypos = parser.linearval('Y', probe.y);
    #else
...
inline void gcode_G92() {
  ...
  bool didE = false;
    ...
    // when setting Z.
    LOOP_XYZE(i) orig_abze[i] = abze[i] = planner.get_axis_position_mm((AxisEnum)i);
    LOOP_XYZ(i) {
...
void process_parsed_command() {
  ...
  switch (parser.command_letter) {
    case 'G': switch (parser.codenum) {
      ...
      #if ENABLED(MAKERARM_SCARA)
        case 6: gcode_G6(); break;                                // G6: SCARA Raw Move to angles AB and position Z
        case 7: gcode_G7(); break;                                // G7: SCARA Kinematic Test
        case 8: gcode_G8(); break;                                // G8: Force machine to be "homed"
        case 9: gcode_G9(); break;                                // G9: Get/Set Arm Orientation
        case 10: gcode_G10(); break;                              // G10: Set the tool offset
      #endif
    ...
    case 'M': switch (parser.codenum) {
      ...
      case 502: gcode_M502(); break;                              // M502: Revert Settings to defaults
        ...
        case 453: gcode_M453(); break;                            // M453: Select CNC printer mode
        case 454: gcode_M454(); break;                            // M454: Select Pick-and-Place printer mode
      #endif
  ...
  inline bool prepare_kinematic_move_to(const float (&cart)[XYZE]) {
    ...
    if (!position_is_reachable(rtarget[X_AXIS], rtarget[Y_AXIS])) return true;
      ...
      if (tool_type == TOOL_TYPE_PICKER) {
        ...
        rtarget[S_AXIS] -= (delta[A_AXIS] + delta[B_AXIS]) - (planner.get_axis_position_mm(A_AXIS) + planner.get_axis_position_mm(B_AXIS));
      }
    #endif
...
void prepare_move_to_destination() {
  ...
  void forward_kinematics_SCARA(const float &a, const float &b) {
    ...
    const float sin_a = sin(RADIANS(a)),
                cos_a = cos(RADIANS(a)),
...
void setup() {
  ...
  lcd_reset_status();
    ...
    */
    ...
    char ssid[] = WIFI_SSID_1, pass[] = WIFI_PASS_1;
