...
void RTSSHOW::RTS_SDCardInit(void) {
  ...
  if (card.cardOK) {
    ...
    // 0 for Ready
    RTS_SndData(0 + (LanguageRecbuf ? 0 : CEIconGrap),IconPrintstatus);
    lcd_sd_status = IS_SD_INSERTED;
  }
  else {
    SERIAL_ECHOLN("***Initing card fails***");
    // 6 for Card Removed
    RTS_SndData(6 + (LanguageRecbuf ? 0 : CEIconGrap),IconPrintstatus);
  }
}
...
int RTSSHOW::RTS_CheckFilament(int mode) {
  waitway = 4;
  // no filaments check
  for (Checkfilenum = 0; (0 == READ(CHECK_FILAMENT)) && (Checkfilenum < 50); Checkfilenum++) {
    delay(15);
  }
  ...
  if (49 <= Checkfilenum) {
    // no filaments
    if (mode) {
      // for mode status of no filament . the sentence can be canceled, which isn't neccessary?
      FilamentStatus[0] = mode;
      if (LanguageRecbuf != 0) {
  ...
  switch(Checkkey) {
    ...
    case PrintChoice:
      if (recdat.addr == Stopprint) {
        ...
        // recover the status waiting to check filaments
        // FilamentStatus[0]  =  0;
        ...
        RTS_SndData(1 + (LanguageRecbuf ? 0 : CEIconGrap),IconPrintstatus);
    ...
    case TempControl:
      if (recdat.data[0] == 0) {
        ...
        delay(1);
        // 8 for Cooling
        RTS_SndData(8 + (LanguageRecbuf ? 0 : CEIconGrap),IconPrintstatus);
        RTS_SndData(ExchangePageBase + (LanguageRecbuf ? 15 : 57), ExchangepageAddr);
    ...
    case Setting:
      if (recdat.data[0] == 0) { // return to main page
        ...
        RTS_SndData(10*ChangeMaterialbuf[0], FilamentUnit1);
    ...
    case Filament:
      #if CHECKFILAMENT
        /**************checking filament status during changing filament************/
        if (RTS_CheckFilament(3)) break;
      #endif
    ...
    case No_Filament:
      ...
      if (recdat.data[0] == 1) {
        /**************checking filament status during changing filament************/
        if (RTS_CheckFilament(0)) break;

        // check filament before starting to print
        ...
        else if (FilamentStatus[0] == 2) {
          // check filaments status during printing
          char pause_str_Z[16];
          char pause_str_E[16];
          memset(pause_str_Z, 0, sizeof(pause_str_Z));
          dtostrf(pause_z, 3, 2, pause_str_Z);
    ...
    case Filename :
      if (card.cardOK && (recdat.addr == FilenameChs)) {
        ...
        if (recdat.data[0] == 1 && card.cardOK) {
          ...
          PrintStatue[1] = 0;
          // close the checking filaments status in printing
          FilamentStatus[1] = 0;
          PrinterStatusKey[0] = 1;
          PrinterStatusKey[1] = 3;
          // open the key of  checking card in  printing
...
void EachMomentUpdate() {
  ...
  if (ms > next_rts_update_ms && InforShowStatus) {
    ...
    if ((power_off_type_yes == 0)  && lcd_sd_status && (power_off_commands_count > 0)) {
      ...
      // statuse of loadfilament and unloadfinement when temperature is less than
...
void RTSUpdate() {
  ...
  #if CHECKFILAMENT
    // checking filament status during printing
    if (FilamentStatus[1] == 2 && true==card.sdprinting) {
      char cmd[2][30];
      if (0 == READ(CHECK_FILAMENT)) {
        Checkfilenum++;
        delay(5);
        if (Checkfilenum>50) {
          if (LanguageRecbuf != 0)
