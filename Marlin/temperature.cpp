...
#include "language.h"
  ...
  #endif
#else
  static void* heater_ttbl_map[1] = { 0 };
  static uint8_t heater_ttbllen_map[1] = { 0 };
#endif
...
uint16_t Temperature::raw_temp_bed_value = 0;
  ...
  uint8_t Temperature::soft_pwm_amount[HOTENDS];
    ...
    int16_t Temperature::meas_shift_index,      // Index of a delayed sample in buffer
            Temperature::current_raw_filwidth = 0;  // Measured filament diameter - one extruder only
  #endif
...
int Temperature::getHeaterPower(const int8_t heater) {
  return heater < 0 ?
    #if HAS_HEATER_BED
      soft_pwm_amount_bed
    #else
  ...
  //
  void Temperature::_temp_error(const int8_t e, const char* serial_msg, const char* lcd_msg) {
    static bool killed = false;
    if (IsRunning()) {
      SERIAL_ERROR_START();
      serialprintPGM(serial_msg);
      SERIAL_ERRORPGM(MSG_STOPPED_HEATER);
      if (e >= 0) SERIAL_ERRORLN(e); else SERIAL_ERRORLNPGM(MSG_HEATER_BED);
    }
  ...
  float Temperature::get_pid_output(const int8_t e) {
    ...
    float pid_output;
      ...
      pid_output =
        #if ENABLED(ADVANCED_PAUSE_FEATURE)
          heater_idle_timeout_exceeded[HOTEND_INDEX] ? 0 :
        #endif
        isHeatingHotend(HOTEND_INDEX) ? PID_MAX : 0
      ;
