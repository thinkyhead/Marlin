...
static void lcd_implementation_status_screen() {
  ...
  // Use PAGE_CONTAINS(row_y1, row_y2) to skip drawing rows not in the current page
  uint8_t row_y1, row_y2;

  // Set the colors for a menu item based on whether it is selected
  static bool lcd_implementation_mark_as_selected(const uint8_t row, const bool isSelected) {

    #define ROW_HEIGHT (DOG_CHAR_HEIGHT + 2 * (TALL_FONT_CORRECTION))
  ...
  static void lcd_implementation_drawmenu_static(const uint8_t row, const char* pstr, const bool center=true, const bool invert=false, const char* valstr=NULL) {
    if (lcd_implementation_mark_as_selected(row, invert)) {
      ...
      if (center) {
        int8_t space = n - lcd_strlen_P(pstr) * (DOG_CHAR_WIDTH);  // pixels of space left
        NOLESS(space, 0);
        lcd_moveto((START_COL) * (DOG_CHAR_WIDTH) + space / 2, row_y2);
      }

      n -= lcd_printstr_P(pstr, n); // print pstr
      lcd_printstr(valstr, n);      // print valstr
    }
  }
  ...
  void lcd_implementation_drawedit(const char* const pstr, const char* const value=NULL) {
    const uint8_t label_width = uxg_GetUtf8StrPixelWidthP(pu8g->getU8g(), pstr),
                  vallen = lcd_strlen(value);

    uint8_t rows = label_width > DOG_CHAR_WIDTH * (LCD_WIDTH - 2 - vallen) ? 2 : 1;

    #if ENABLED(USE_BIG_EDIT_FONT)
      uint8_t lcd_width, char_width;
      if (label_width <= DOG_CHAR_WIDTH * (LCD_WIDTH_EDIT) - 1) {
        if (label_width >= LCD_PIXEL_WIDTH - DOG_CHAR_WIDTH * vallen) rows = 2;
    ...
    if (value != NULL) {
      ...
      if (PAGE_CONTAINS(baseline + 1 - (DOG_CHAR_HEIGHT_EDIT), baseline)) {
        lcd_print_u(':');
        lcd_moveto((lcd_width - 1 - vallen) * char_width, baseline);
        lcd_print(value);
      }
    }
  }
    ...
    static void _drawmenu_sd(const bool isSelected, const uint8_t row, const char* const pstr, const char* filename, char* const longFilename, const bool isDir) {
      ...
      if (lcd_implementation_mark_as_selected(row, isSelected)) {
        ...
        if (isDir) {
          lcd_print_u(LCD_STR_FOLDER[0]);
          --n;
        }

        lcd_printstr(filename, n * DOG_CHAR_WIDTH);
      }
    }
